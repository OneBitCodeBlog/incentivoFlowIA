...
const calculateCampaignStats = async (campaignId) => {
    ...
    // Contar os leads que baixaram a primeira recompensa e foram convidados por outro lead
    const leadsWhoClaimedFirstRewardAndWereInvited = await LeadCampaign.count({
        where: { 
            campaign_id: campaignId,
            first_reward_claimed: true,
            invited_by_lead_id: { [Op.ne]: null }  // Verifica se o lead foi convidado
        }
    });

    const conversionRate = totalLeads > 0 ? (leadsWhoClaimedFirstRewardAndWereInvited / totalLeads) * 100 : 0;

    return {
        totalLeads,
        leadsWhoClaimedFirstRewardAndWereInvited,
...
const campaignResourceOptions = {
    properties: {
        ...
        leadsWhoClaimedFirstRewardAndWereInvited: {
            position: 33,
            isVisible: { list: true, filter: false, show: true, edit: false },
            description: 'Convidados que baixaram recompensa',
        },
        conversionRate: {
            position: 34,
            isVisible: { list: true, filter: false, show: true, edit: false },
    ...
    listProperties: ['slug', 'title', 'status', 'totalLeads', 'leadsWhoClaimedFirstRewardAndWereInvited', 'conversionRate'],
    actions: {
        list: {
            after: async (response, context) => {
                ...
                for (const record of records) {
                    ...
                    record.params.leadsWhoClaimedFirstRewardAndWereInvited = stats.leadsWhoClaimedFirstRewardAndWereInvited;
